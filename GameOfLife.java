...
public class GameOfLife {
	...
	public static int cellValue(int[][] board, int i, int j) {
		...
		if (cellValue == 1) {
			if (aliveNeighbors < 2) cellValue = 0;
			else if (aliveNeighbors > 3) cellValue = 0;
		}
		else if (cellValue == 0 && aliveNeighbors == 3) cellValue = 1;
	...
	public static int count(int[][] board, int i, int j) {
		...
		int aliveNeighbors = board[i-1][j-1] + board[i][j-1] + board[i+1][j-1]
